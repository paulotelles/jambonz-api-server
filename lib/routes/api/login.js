...
router.post('/', async(req, res) => {
  const {logger, retrieveKey, incrKey, addKey} = req.app.locals;
  ...
  try {
    ...
    const logginAttempsBlocked = await retrieveKey(r[0].user_sid) >= 6;
    ...
    if (!isCorrect) {
      ...
      const attempt = await addKey('1');
      logger.info(`Failed login attempt for user ${attempt}`);
      const newAttempt = await incrKey(r[0].user_sid, attempTime);
      if (newAttempt >= 6) {
        return res.status(403)
